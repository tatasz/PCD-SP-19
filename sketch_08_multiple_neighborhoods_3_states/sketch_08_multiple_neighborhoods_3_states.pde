...
int b0[] = {1, 11, 12, 12};
int b1[] = {11, 81, 312, 326, 391, 508, 508, 515, 545, 545, 545, 548}; 
//vetor que contem as regras para cada intervalo
int regra[] = {2, 1, 1, 1, 2, 1, 0, 1, 1};
...
void setup() {
  ...
  for (int i = 0; i <size; i++) {
    for (int j = 0; j < size; j++) {
      A[i][j] = (random(1) < 0.05) ? floor(random(3)) : 0;
      B[i][j] = 0;
    }
  }
  ...
  for (int i = 0; i <size; i++) {
    for (int j = 0; j < size; j++) {
      pixels[i + j * width] = color(A[i][j] * 255 / 2);
    }
  }
  updatePixels();
}
  ...
  for (int i = 0; i <size; i++) {
    ...
    for (int j = 0; j < size; j++) {
      ...
      //aplicamos as regras
        ...
        if (soma0 >= b0[2] & soma0 <= b0[3]){
          outval = regra[2];
        } 
        if (soma1 >= b1[6] & soma1 <= b1[7]){
          outval = regra[3];
        } 
        if (soma1 >= b1[2] & soma1 <= b1[3]){
          outval = regra[4];
        } 
        if (soma1 >= b1[8] & soma1 <= b1[9]){
          outval = regra[5];
        } 
        if (soma1 >= b1[4] & soma1 <= b1[5]){
          outval = regra[6];
        } 
        if (soma1 >= b1[0] & soma1 <= b1[1]){
          outval = regra[7];
        } 
        if (soma1 >= b1[10] & soma1 <= b1[11]){
          outval = regra[8];
        }   
